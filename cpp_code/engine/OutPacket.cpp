...
OutPacket* OutPacket::putLong(double v) {
...
OutPacket* OutPacket::putFloat(float v) {
	unsigned char* byteArray = reinterpret_cast<unsigned char*>(&v);
	int i;
	for (i = 0; i<4; i++)
	{
		putByte(byteArray[3 - i]);
	}
	return this;
}
